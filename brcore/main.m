...
void run() {
    ...
    br_server_create("0.0.0.0", "9999", ^(br_client_t *c) {
        count++;
        br_log_info("ACCEPT CLIENT number %d socket %d: %s:%s", count, c->fd, c->hbuf, c->sbuf);
    }, ^(br_client_t *c, char *buff, size_t buff_len) {
        br_log_info("CLIENT READ socket %d bytes %d", c->fd, (int)buff_len);
        ...
        br_client_write(c, buff2, buff2_len, ^(br_client_t *c) {
            br_log_info("CLIENT ERROR on write socket %d", c->fd);
        });
        br_log_info("CLIENT WRITE %d", c->fd);
        br_client_close(c);
    }, ^(br_client_t *c) {
        br_log_info("CLIENT CLOSED %d", c->fd);
    });
